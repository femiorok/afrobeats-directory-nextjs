...
const playSong = (num) => {
    ...
    const searchTerms = () => {
    if (num === 1) {
        return [songSeed1?.trackMetadata.trackName, songSeed1?.trackMetadata.artists[0].name]
    }
    if (num === 2) {
        return [songSeed2?.trackMetadata.trackName, songSeed2?.trackMetadata.artists[0].name]
    }
    if (num === 3) {
        return [songSeed3?.trackMetadata.trackName, songSeed3?.trackMetadata.artists[0].name]
    }
}
...
const lookUpSongId = async () => {
    ...
    const query = new URLSearchParams(`${_searchTerms[0]} ${_searchTerms[1]}`).toString()
    ...
    fetch(`https://youtube-music1.p.rapidapi.com/v2/search?query=${query}`, options)
        .then(response => response.json())
  ...
  return (
    ...
    {/* First Card */}
        <div className='flex flex-col items-center'>
            ...
            <img className="object-cover h-full w-full" src={songSeed1?.trackMetadata.displayImageUri}></img>
            ...
            <h1>{songSeed1?.trackMetadata.trackName} - {songSeed1?.trackMetadata.artists[0].name}</h1>
            </div>
            </div>
            <div className='flex justify-between w-full mt-4'>
            <button onClick={() => generateRandomIndex(1)} className="relative p-0.5 inline-flex items-center justify-center font-bold overflow-hidden group rounded-md">
            ...
            <button onClick={() => playSong(1)} className="relative p-0.5 inline-flex items-center justify-center font-bold overflow-hidden group rounded-md">
            <span className="w-full h-full bg-gradient-to-br from-red-800 via-yellow-600 to-yellow-500 group-hover:from-red-800 group-hover:via-yellow-600 group-hover:to-yellow-500 absolute"></span>
            <span className="relative px-3 py-3 transition-all ease-out bg-gray-900 rounded-md group-hover:bg-opacity-0 duration-400">
        ...
        <div className="song-card w-[300px] h-[400px] flex-col justify-between items-center relative">
            <img className="object-cover h-full w-full" src={songSeed2?.trackMetadata.displayImageUri}></img>
            <div className="footer w-full backdrop-blur-sm absolute bottom-0 text-center text-xl font-bold text-slate-100 tracking-wide bg-slate-900/40 px-2">
            <h1>{songSeed2?.trackMetadata.trackName} - {songSeed2?.trackMetadata.artists[0].name}</h1>
            </div>
            </div>
            <div className='flex justify-between w-full mt-4'>
            <button onClick={() => generateRandomIndex(2)} className="relative p-0.5 inline-flex items-center justify-center font-bold overflow-hidden group rounded-md">
            <span className="w-full h-full bg-gradient-to-br from-red-800 via-yellow-600 to-yellow-500 group-hover:from-red-800 group-hover:via-yellow-600 group-hover:to-yellow-500 absolute"></span>
            <span className="relative px-3 py-3 transition-all ease-out bg-gray-900 rounded-md group-hover:bg-opacity-0 duration-400">
            <span className="relative text-white">Update selection</span>
            </span>
            </button>
            <button onClick={() => playSong(2)} className="relative p-0.5 inline-flex items-center justify-center font-bold overflow-hidden group rounded-md">
            <span className="w-full h-full bg-gradient-to-br from-red-800 via-yellow-600 to-yellow-500 group-hover:from-red-800 group-hover:via-yellow-600 group-hover:to-yellow-500 absolute"></span>
            <span className="relative px-3 py-3 transition-all ease-out bg-gray-900 rounded-md group-hover:bg-opacity-0 duration-400">
            <span className="relative text-white">Play Song</span>
            </span>
        ...
        <div className='flex flex-col items-center'>
            <div className="song-card w-[300px] h-[400px] flex-col justify-between items-center relative">
            <img className="object-cover h-full w-full" src={songSeed3?.trackMetadata.displayImageUri}></img>
            <div className="footer w-full backdrop-blur-sm absolute bottom-0 text-center text-xl font-bold text-slate-100 tracking-wide bg-slate-900/40 px-2">
            <h1>{songSeed3?.trackMetadata.trackName} - {songSeed3?.trackMetadata.artists[0].name}</h1>
            </div>
            </div>
            <div className='flex justify-between w-full mt-4'>
            <button onClick={() => generateRandomIndex(3)} className="relative p-0.5 inline-flex items-center justify-center font-bold overflow-hidden group rounded-md">
            <span className="w-full h-full bg-gradient-to-br from-red-800 via-yellow-600 to-yellow-500 group-hover:from-red-800 group-hover:via-yellow-600 group-hover:to-yellow-500 absolute"></span>
            <span className="relative px-3 py-3 transition-all ease-out bg-gray-900 rounded-md group-hover:bg-opacity-0 duration-400">
            <span className="relative text-white">Update selection</span>
            </span>
    ...
    {loadingSong ? <LoadingVideo ref={video} onLoad={() => scroll()}/> : <YoutubePlayer songId={songId} ref={video} onLoad={() => scroll()} />} 
    <button onClick={() => getRecs()} className="relative p-0.5 inline-flex items-center justify-center font-bold overflow-hidden group rounded-md">
            <span className="w-full h-full bg-gradient-to-br from-red-800 via-yellow-600 to-yellow-500 group-hover:from-red-800 group-hover:via-yellow-600 group-hover:to-yellow-500 absolute"></span>
            <span className="relative px-3 py-3 transition-all ease-out bg-gray-900 rounded-md group-hover:bg-opacity-0 duration-400">
...
export async function getStaticProps() {
    const res = await fetch('https://afrobeats-library-default-rtdb.firebaseio.com/afrobeatsdata/-NBMH0XrzsDzd4O1sNrd.json')
